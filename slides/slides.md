...
<!--
As we all have seen in the last couple of years, we're moving into a new future for web applications driven. Do you know what I'm talking about?
...
<!--
Then, we have Server Side Rendering. We still have our client component, but we can prepare our code to fetch product's data from the database on our server, and render the component as a string.
...
<<< @/../04_react-rsc-router/src/App.js#app jsx
...
<!--
Or, we could make it dynamic. I know the user's session handling the request, I prepare their own response and serve it.
...
I can generate a static shell of my page with some placeholders, I can stream dynamic data as soon as they are ready to show the cart quantity icon, and finally I can fetch additional less relevant data on the client.
...
It creates a memoized function with invalidation tags and an expiration time.
-->
...
<!-- This is the cart of all cache layers introduced by Next.js to push to top level performances, but again you have all the basics to make your own engineering cache model -->
...
This means we run server-side code just invoking a function.
...
Our CEO asks to us to have the animated snowflakes on the header and to make them happy we change the Header into a client component to install some random lib.
-->
...
- ... but also introduces a new kind of _fullstack_ problems to face, and everything went so fast

React 19 and the ecosystem around is growing fast in every possible direction, providing a lot of different features.
If before we had a switch for on server-side or  onclient-side code, now we have a slider where we can easily specify what should run on client and what on server.
As we saw, involving servers means a new wave of problems that must be faced, and the creation of best practices and solutions for those problems are the new challenges of this new fullstack era.
-->
